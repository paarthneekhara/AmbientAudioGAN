...
TRAIN_DIR=/data2/paarth/TrainDir/Ambient/tatum_dp_skip
rm -rf ${TRAIN_DIR}
python3 train_evaluate.py train \
	${TRAIN_DIR} \
	--data_dir /data2/paarth/ambient/clipped_dp_512_04/tatum/train \
	--data_fastwav \
	--model_overrides "objective=l1,batchnorm=False,train_batch_size=64,alpha=100.0,enc_length=64" \
	--train_summary_every_nsecs 60 

export CUDA_VISIBLE_DEVICES="0"
TRAIN_DIR=/datasets/home/10/610/pneekhar/TRAIN/Ambient/sc09_dp_skip
python3 train_evaluate.py eval \
	${TRAIN_DIR} \
	--data_dir /datasets/home/10/610/pneekhar/DATA/clipped_dp_256_04/sc09/valid \
	--data_fastwav \
	--model_overrides "objective=l1,batchnorm=False,train_batch_size=2,alpha=100.0,enc_length=64"
